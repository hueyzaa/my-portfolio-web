...
export const UpdatePasswordForm: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    setLoading(true);
    try {
      const resp: IApiSuccess = await apiInstance.patch(`${API_URL.UPDATE_PASS}`, {
  ...
  if (!user?.need_update_password) return <Navigate to={'/profile'} />;
      ...
      <BaseRow gutter={[10, 10]}>
        <BaseCol span={24}>
          <BaseButtonsForm.Item>
            <BaseButtonsForm.Title>{'Cập nhật mật khẩu'}</BaseButtonsForm.Title>
          </BaseButtonsForm.Item>
        </BaseCol>
